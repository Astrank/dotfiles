...
function Metronome() {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      ...
      // If the metronome is not playing, clear the timeout
  ...
  function playClick() {
    ...
    // Set the oscillator type to 'sine' and the frequency to 440 Hz
    oscillator.type = 'sine';
    oscillator.frequency.value = 440;

    // Calculate the time at which the sound should be played, in seconds
    const now = audioContext.currentTime;
    const time = now + 0.1;  // Play the sound after a short delay

    // Set the gain to a high value at the scheduled time to play the sound
    ...
    // After a short time, stop the oscillator and reset the gain to 0
    setTimeout(() => {
      oscillator.stop();
    ...
    const nextTime = now + 60 / tempo;  // Calculate the time for the next click
    setTimeout(playClick, nextTime - audioContext.currentTime);  // Schedule the next click using the Web Audio API's setTimeout function
  }
