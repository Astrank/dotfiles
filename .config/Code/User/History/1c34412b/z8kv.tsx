...
const CommentComponent = ({ comment, replies, user, isAdmin, deleteComment, likeComment }: CommentProps) => {
    ...
    return (
        <div className='flex flex-col gap-2 mt-8 group' key={comment.id}>
            <CommentStructure comment={comment} likeComment={likeComment} deleteComment={deleteComment} />
            
            {replies && replies.length > 0 && 
                <button className="font-bold self-start" onClick={() => toggleReplies(!showingReplies)}>{replies.length == 1 ? `${replies.length} respuesta` : `${replies.length} respuestas` }</button>}
