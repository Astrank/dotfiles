...
const EscrituraGrupal: NextPage<EscrituraGrupalProps> = ({ escritura }) => {
  const { getStories, approveStory, unsubscribe, stories } = useEscrituraGrupal();
  ...
  return (
    <div className="min-h-screen text-primary-900 ">
      ...
      <section className="flex flex-col mx-4 md:mx-10 lg:mx-14 lg:my-20 lg:mx-44">
        <textarea onChange={(e) => setFragment(e.target.value)} name="" id="" rows={5} className="p-2 text-primary-700 outline-none border border-primary-700" placeholder="Continuación de la historia" />
        <button 
          ...
          {stories
            ...
            .map(story => (
              <div>
                ...
                <button onClick={() => approveStory(story.authorId, story.slug)}>Aprobar</button>
              </div>
          ))}
        </section>}
