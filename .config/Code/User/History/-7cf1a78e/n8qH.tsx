...
export default function Contacto() {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, { resetForm }) => {
            fetch('https://formsubmit.co/ajax/a', {
                method: 'POST',
                headers: {
            ...
            {toast &&
                <Toast.Provider duration={5000}>
                    <Toast.Root 
                        ...
                        className={`flex items-center text-white py-6 px-8 rounded 
                                    ${toast.status == "success" ? "bg-green-400" 
                                        : toast.status == "error" ? "bg-error-300" 
                                        : "bg-primary-900"}`}>
                            <Toast.Title className="mr-8">{toast.title}</Toast.Title>
                            <Toast.Close>
                                <svg className={`w-6 h-6 rounded hover:bg-primary-700`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
