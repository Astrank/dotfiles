    ...
    return (
        <div className='flex flex-col gap-4 group' id={comment.id} key={comment.id}>
            ...
            <Dialog open={deleteCommentAlert} onClose={() => toggleDeleteCommentAlert(false)}>
                <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
                <div className='fixed inset-0 flex items-center justify-center p-4'>
                    <Dialog.Panel className="flex flex-col gap-6 p-4 text-sm bg-primary-900 text-white rounded">
                            <span>¿Elminar comentario permanentemente?</span>
                            <div className='flex gap-2 self-end'>
                                <button className='text-neutral-700' onClick={() => toggleDeleteCommentAlert(false)}>Cancelar</button>
                                <button className='text-secondary-600' onClick={() => deleteComment(comment.id, comment.post)}>Confirmar</button>
                            </div>
                    </Dialog.Panel>
                </div>
            </Dialog>
