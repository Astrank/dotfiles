                    ...
                    <div>
                        ...
                        {isAdminBoxOpen &&
                            <div className="absolute mt-2 flex flex-col justify-start py-1 text-sm rounded overflow-hidden bg-neutral-800 text-white right-6 md:right-12 lg:right-44">
                                <button className='px-3 py-2 hover:bg-neutral-700' onClick={() => {toggleDeleteCommentAlert(true)}}>Eliminar Comentario</button>
                                <Dialog open={deleteCommentAlert} onClose={() => toggleDeleteCommentAlert(false)}>
                                    <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
                                    <div className='fixed inset-0 flex items-center justify-center p-4'>
                                        <Dialog.Panel className="flex flex-col gap-6 p-4 text-sm bg-white rounded">
                                                <span>¿Elminar comentario permanentemente?</span>
                                                <div className='flex gap-2 text-secondary-600 self-end'>
                                                    <button onClick={() => toggleDeleteCommentAlert(false)}>Cancelar</button>
                                                    <button onClick={() => deleteComment(comment.id, comment.post)}>Confirmar</button>
                                                </div>
                                        </Dialog.Panel>
                                    </div>
                                </Dialog>
