...
export function EscrituraGrupalProvider({ children }: {children : React.ReactNode}) {
    ...
    const postStory = async (slug: string, fragment: string) => {
        if(user) {
            try {
    ...
    return <EscrituraGrupalContext.Provider value={{ getStories, approveStory, stories, unsubscribe }}>{children}</EscrituraGrupalContext.Provider>;
}
