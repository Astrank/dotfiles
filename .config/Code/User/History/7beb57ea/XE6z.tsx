...
export function CommentsProvider({ children }: {children : React.ReactNode}) {
            ...
            try {
                ...
                if (user.uid !== uid) {
                    if (replyingTo !== uid && replyingTo !== user.uid) {
                        await setDoc(doc(db, "notifications", uid, "comment", docRef.id), {
                            notification: " ha respondido en ",
                            userId: user.uid,
                            author: user.displayName,
                            comment: comment,
                            post: slug,
                        ...
                        notifyAdmin(docRef.id, comment, slug, post)
                        notifyUser(replyingTo, docRef.id, comment, slug, post)

                        await setDoc(doc(db, "notifications", replyingTo, "comment", docRef.id), {
                            notification: " ha respondido en ",
                            userId: user.uid,
                            author: user.displayName,
                            comment: comment,
    ...
    const notifyAdmin = async (commentId: string, comment: string, postSlug: string, postTitle: string) => {
    ...
    const notifyUser = async (replyingTo: string, commentId: string, comment: string, postSlug: string, postTitle: string) => {
        await setDoc(doc(db, "users", replyingTo, "comment", commentId), {
            notification: " ha comentado en ",
            userId: user.uid,
            author: user.displayName,
            comment: comment,
