...
export default function Home({ posts }) {
    ...
    return (
        <div className="flex flex-col lg:flex-row text-gray-700 bg-white dark:bg-dark dark:text-neutral-200 transition duration-700 divide-x-none lg:h-screen lg:overflow-hidden">
            <nav className="w-full lg:w-96 flex flex-row py-8 transition duration-700 lg:border-r lg:border-gray-300 dark:lg:border-none dark:lg:bg-neutral-600 lg:flex-col gap-4 px-3 sm:px-6 md:px-14 lg:overflow-y-auto lg:px-6 justify-between lg:justify-start">
                <Switcher />
                ...
                <div className="hidden lg:flex lg:flex-col lg:gap-4">
                    {posts &&
                        posts.map((post, index) => (
                            <Link
                                className="cursor-pointer text-sm font-classic font-semibold hover:text-blue-400 hover:underline"
                                key={post.slug}
                                href={`${post.slug}`}
                                onClick={() => {
                                    setCurrent(index);
